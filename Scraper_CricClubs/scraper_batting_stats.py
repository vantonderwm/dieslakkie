...
def parse_batting_from_html_file(html_file_path):
    ...
    # Find the batting table
    table = soup.find('table', {'id': 'tableBattingRecords'})
    if not table:
        print("Batting table with id 'tableBattingRecords' not found.")
        return None
    ...
    if data:
        ...
        print(f"Successfully retrieved {len(df)} batsmen from the HTML file.")
        return df
    else:
...
if __name__ == "__main__":
    ...
    df = parse_batting_from_html_file(html_file_path)
    if df is not None:
        ...
        expected_columns = ["SNO","PLAYER","TEAM_IMG","BLANK","TEAM","MAT","INS","NO","RUNS","BALLS","AVG","SR","HS","100s", "75s", "50s", "25s", "0s", "6s", "4s"]
        ...
        # Store data
        df.to_csv("scrape_batting_stats.csv", index=False)

        print("Data saved to scrape_batting_stats.csv")