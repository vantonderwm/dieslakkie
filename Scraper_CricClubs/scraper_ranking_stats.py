...
def parse_ranking_from_html_file(html_file_path):
    ...
    table = soup.find('table', {'id': 'tablePlayerRankings'})
    if not table:
        print("Ranking table with id 'tableRankingRecords' not found.")
        return None
    ...
    if data:
        ...
        print(f"Successfully retrieved {len(df)} ranked players from the HTML file.")
        return df
    else:
...
if __name__ == "__main__":
    ...
    df = parse_ranking_from_html_file(html_file_path)
    if df is not None:
        ...
        # Store data
        df.to_csv("scrape_ranking_stats.csv", index=False)

        print("Data saved to scrape_ranking_stats.csv")