...
def parse_bowling_from_html_file(html_file_path):
    ...
    # Find the bowling table
    table = soup.find('table', {'id': 'tableBowlingRecords'})
    if not table:
        print("Bowling table with id 'tableBowlingRecords' not found.")
        return None
    ...
    if data:
        ...
        print(f"Successfully retrieved {len(df)} bowlers from the HTML file.")
        return df
    else:
...
if __name__ == "__main__":
    ...
    df = parse_bowling_from_html_file(html_file_path)
    if df is not None:
        ...
        expected_columns = ["SNO","PLAYER","TEAM_IMG","BLANK","MAT", "TEAM","INS","OVERS","RUNS","WKTS","BBF","MDNS","DOTS","ECON", "AVE", "SR", "HATTRICK", "4W", "5W", "WIDES", "NB"]
        ...
        # Store data
        df.to_csv("scrape_bowling_stats.csv", index=False)

        print("Data saved to scrape_bowling_stats.csv")